...
def evaluate_cos(model, dataloader, epoch):
    ...
    r_at_2 = calc_recall_at_k(T, Y, 2)
    r_at_4 = calc_recall_at_k(T, Y, 4)
    r_at_8 = calc_recall_at_k(T, Y, 8)
    recall.append(r_at_k)
    print("R@{} : {:.3f}".format(1, 100 * r_at_k))
    # print("R@{} : {:.3f}".format(2, 100 * r_at_2))
    # print("R@{} : {:.3f}".format(4, 100 * r_at_4))
    # print("R@{} : {:.3f}".format(8, 100 * r_at_8))
    return recall
...
def show_OnN(m, y, v, nb_classes, pth_result, thres=0., is_hist=False, iter=0):
    ...
    print('acc: {}'.format((oo_i + nn_i) / (oo_i + on_i + no_i + nn_i)))
    precision_n = nn_i / (on_i + nn_i) if on_i + nn_i != 0 else 0
    precision_o = oo_i / (oo_i + no_i) if oo_i + no_i != 0 else 0
    recall_n = nn_i / (nn_i + no_i) if nn_i + no_i != 0 else 0
    recall_o = oo_i / (oo_i + on_i) if oo_i + on_i != 0 else 0
