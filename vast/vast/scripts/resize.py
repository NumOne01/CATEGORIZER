...
parser.add_argument(
    "--org_dataset_path",
    help="Path to original images to resize",
    default="/net/kato/store2/ImageNet/ILSVRC_2012/val_in_folders/",
)
parser.add_argument(
    "--resized_dataset_path",
    help="Path to resized images directory. Would be created if it doesn't exist",
    default="/net/kato/store2/ImageNet/ILSVRC_2012/val/",
)
parser.add_argument(
    "--resized_image_height", help="resized_image_height", type=int, default=256
)
parser.add_argument(
    "--resized_image_width", help="resized_image_width", type=int, default=256
)
...
def process_each_file(file_name):
    ...
    image = cv2.resize(
        img,
        (args.resized_image_height, args.resized_image_width),
...
print(
    f"Total number of images for which src was smaller than destination {images_with_src_smaller_than_dest}/{len(all_images_to_process)}"
)
