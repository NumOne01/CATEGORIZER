...
class ExternalSourcePipeline(Pipeline):
    def __init__(
        ...
        super(ExternalSourcePipeline, self).__init__(
            ...
            device_id,
            seed=34,
            prefetch_queue_depth={"cpu_size": 10, "gpu_size": 2},
        )
        self.input = nvidia_ops.ExternalSource()
        self.input_label = nvidia_ops.ExternalSource()
        self.training = training
        self.crop_loc = nvidia_ops.Uniform(range=(0.0, 1.0))
        self.anchor_loc = nvidia_ops.Uniform(range=(0.0, 1.0), shape=(4,))
        self.shape_loc = nvidia_ops.Uniform(range=(0.2, 0.5), shape=(4,))
        self.coin = nvidia_ops.CoinFlip(probability=0.5)
        self.rotation_angle = nvidia_ops.Uniform(range=(-45.0, 45.0))
        self.train_decode = nvidia_ops.ImageDecoderRandomCrop(
            device="mixed" if device_type == "gpu" else "cpu",
            device_memory_padding=211025920,
            host_memory_padding=140544512,
            output_type=nvidia_types.RGB,
            random_aspect_ratio=[0.5, 4.0],
            random_area=[0.01, 1.0],
            num_attempts=100,
        )
        self.val_decode = nvidia_ops.ImageDecoder(
            device="mixed" if device_type == "gpu" else "cpu", output_type=nvidia_types.RGB
        )
        self.rotate = nvidia_ops.Rotate(device=device_type)
        self.erase = nvidia_ops.Erase(
            device=device_type,
            axis_names="HW",
        ...
        self.resize = nvidia_ops.Resize(
            device=device_type, resize_shorter=(image_size * 256 / 224)
        )
        self.crop_mirror_normalize = nvidia_ops.CropMirrorNormalize(
            device=device_type,
    ...
    def training_data_augmentation(self):
        ...
        images = self.crop_mirror_normalize(
            images,
            crop_pos_x=self.crop_loc(),
            crop_pos_y=self.crop_loc(),
            mirror=self.coin(),
        )
